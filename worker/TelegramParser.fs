...
module ClientFactory =
    ...
    let make host port =
        let proxyClient =
            Socks5ProxyClient(Configs.proxy.Split(':').[0], Configs.proxy.Split(':').[1] |> int, Configs.auth.Split(':').[0], Configs.auth.Split(':').[1])
        proxyClient.CreateConnection(host, port)

    let mkClient() =
        if isNull Configs.proxy
        then new TelegramClient(Configs.appId, Configs.apiHash)
    ...
    { new TelegramConnectorApiImpl' with
        ...
        member __.updateToken code =
            async {
                ...
                let! r = client.MakeAuthAsync(Configs.phone, hash, code)
                         |> Async.AwaitTask
                         |> Async.Catch
        ...
        member __.getNodes uri =
            async {
                ...
                return (history :?> TLChannelMessages).Messages
                       |> Seq.choose ^ function
