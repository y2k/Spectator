...
module Domain =
    ...
    let private matchContent content optRegex =
        if String.isNullOrEmpty optRegex then true else R.IsMatch(content, optRegex)

    let private getUpdates (subscriptions : Subscription list) snaps =
        subscriptions
        |> List.map ^ fun sub -> sub, snaps |> List.filter ^ fun x -> x.subscriptionId = sub.id && matchContent x.title sub.filter
        |> List.filter ^ fun (_, snaps) -> snaps |> (List.isEmpty >> not)

    let handleUpdate db =
        let botCommands =
            getUpdates db.subscriptions db.snapshots.unwrap |> List.map ^ fun (sub, snaps) -> sub.userId, mkMessage sub snaps
        botCommands
...
let main env mdb =
    ...
    M.subscribeQuery mdb ^ fun db -> Domain.handleUpdate db |> sendToTelegramBroadcast
