...
type Log() =
    static member log (message : string,
                       [<System.Runtime.CompilerServices.CallerFilePath; System.Runtime.InteropServices.Optional; System.Runtime.InteropServices.DefaultParameterValue("")>] file : string,
                       [<System.Runtime.CompilerServices.CallerLineNumber; System.Runtime.InteropServices.Optional; System.Runtime.InteropServices.DefaultParameterValue(0)>] line : int) =
...
module MongoCofx =
    ...
    module Effects =
        let insert (db : IMongoDatabase) colName items =
            async {
                do! items
                    |> List.map ^ fun x -> Async.wrapTask ^ fun _ -> (db.GetCollection colName).InsertOneAsync <| x.ToBsonDocument()
                    |> Async.seq
                    >>- List.iter (sprintf "Error %O" >> Log.log)
            }

        let query (mdb : IMongoDatabase) colName limit offset =
            mdb.GetCollection(colName).Find(FilterDefinition.op_Implicit "{}").Sort(SortDefinition.op_Implicit "{$natural:-1}")
               .Limit(limit |> Option.toNullable).Skip(offset |> Option.toNullable).ToListAsync()
            |> Async.AwaitTask
            >>- Seq.toList
        ...
        async {
            ...
            try
                ...
                if newDb.newSubscriptions <> db.newSubscriptions then
                    ...
                    do! newDb.newSubscriptions |> Effects.insert mongo R.NewSubscriptionsDb
                if newDb.subscriptions <> db.subscriptions then
                    do! Effects.deleteFromCol mongo R.SubscriptionsDb
                    do! newDb.subscriptions |> Effects.insert mongo R.SubscriptionsDb
                let (EventLog snapshots) = newDb.snapshots
                if not <| List.isEmpty snapshots then do! Effects.insert mongo R.SnapshotsDb snapshots
                return eff
            finally
    ...
    let subscribeQuery (mdb : IMongoDatabase) f =
        async {
            ...
            while true do
                ...
                if not <| List.isEmpty snaps then
                    do! runCfx mdb (fun db -> db, f { db with snapshots = EventLog snaps }) >>= id
